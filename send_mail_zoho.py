...
class EmailSender:
    def __init__(self, client_id, client_secret, refresh_token):
        ...
        self.accounts_api = "https://mail.zoho.com/api/accounts"
    ...
    def _get_access_token(self):
        ...
        if not token:
            raise Exception(f"Không lấy được access token: {resp.text}")
    ...
    def _get_account_id(self):
        """Trả về accountId, cache lần đầu"""
        if self._account_id:
            return self._account_id
        token = self._get_access_token()
        headers = {'Authorization': f'Zoho-oauthtoken {token}'}
        resp = requests.get(self.accounts_api, headers=headers)
        resp.raise_for_status()
        data = resp.json().get('data', [])
        if not data:
            raise Exception("Không tìm thấy account Zoho Mail")
        self._account_id = data[0]['accountId']
        return self._account_id
